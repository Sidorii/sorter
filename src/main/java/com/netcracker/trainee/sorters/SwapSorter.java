...
/**
 * Abstract class {@link SwapSorter} is child from {@link Sorter} and used as parent or interface
 * for sorting algorithms that swap elements durring sorting processing.
 *
...
abstract public class SwapSorter extends Sorter {
    ...
    /**
     * Swapping position of two elements in array. Result of this method is array of int where element
     * with index1 will be on index2 position as well as index2 will be on index1 position.
     *
     * @param array  the array of ints which contains elements with indexes {@code index1}
     *               and {@code index2} that'll be swapped.
    ...
    protected final void checkIndexInBounds(int[] array, int index) {
        ...
        if (index < 0 || index >= size) {
            throw new IndexOutOfBoundsException("Index out of bounds while processing swap operation " +
                    "in array with size = " + size +
